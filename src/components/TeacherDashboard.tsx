...
import { fetchPublishedQuizzes, fetchSessionsForQuiz } from '../db'
...
export default function TeacherDashboard({ onBack }: Props) {
  ...
  const [pendingQuiz, setPendingQuiz] = useState<PublishedQuiz | null>(null)
  ...
  async function unlockQuiz(e: React.FormEvent) {
    e.preventDefault()
    if (!pendingQuiz || !password.trim()) return
    setError(null)
    try {
      const data = await fetchSessionsForQuiz(pendingQuiz.access_code, password)
      setSessions(data)
      setSelectedQuiz(pendingQuiz)
      setPendingQuiz(null)
      setPassword('')
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Falsches Passwort')
    }
  }
  ...
  if (pendingQuiz) {
    return (
      <div className="max-w-md mx-auto">
        <button
          onClick={() => { setPendingQuiz(null); setPassword(''); setError(null) }}
          className="text-blue-600 hover:text-blue-800 font-medium mb-4 inline-flex items-center gap-1"
        >
          &larr; Zurück
        </button>
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <h3 className="text-xl font-bold text-slate-800 mb-1">{pendingQuiz.title}</h3>
          <p className="text-sm text-slate-500 mb-4">
            Code: <span className="font-mono font-bold">{pendingQuiz.access_code}</span>
          </p>
          <form onSubmit={unlockQuiz} className="space-y-4">
            <div>
              <label htmlFor="dash-pw" className="block text-sm font-medium text-slate-700 mb-1">
                Lehrer-Passwort
              </label>
              <input
                id="dash-pw"
