...
import { fetchClasses, createClass, updateClass, deleteClass } from '../../db'
...
export default function ClassesPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setError(null)
    try {
      await createClass(newName, user.id)
      setNewName('')
      await loadClasses()
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Fehler')
  ...
  async function handleDelete(id: string, name: string) {
    if (!confirm(`Klasse "${name}" wirklich löschen? Tests behalten die Klasse nicht mehr.`)) return
    setError(null)
    try {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Klassen verwalten</h1>
